...
```bash
# 先把代码拉下来
git clone https://github.com/yourname/dynamic\\\\\\\_memory\\\\\\\_go.git
cd dynamic\\\\\\\_memory\\\\\\\_go
...
storage:
  ...
  mysql:
    dsn: "user:password@tcp(127.0.0.1:3306)/dynamic\\\\\\\_memory?charset=utf8mb4\\\\\\\&parseTime=True\\\\\\\&loc=Local" # 你的MySQL地址
```
...
class MyMetaCalculator(BaseMetaCalculator):
    def calculate\\\\\\\_weight(self, raw\\\\\\\_input: str) -> float:
        # 这里写你自己的权重计算逻辑
        return my\\\\\\\_weight
```
